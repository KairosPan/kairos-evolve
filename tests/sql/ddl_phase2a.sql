-- Phase 2A test schema (interim; Plan 2B replaces with Alembic migration in main).
-- Covers exactly the tables Phase 2A needs:
...
-- because routing_events do not FK into kairos.runs in this phase).
...
CREATE TABLE kairos_audit.audit_log (
    id              UUID PRIMARY KEY,
...
CREATE INDEX idx_audit_log_target ON kairos_audit.audit_log (target_schema, target_table, target_id);
CREATE INDEX idx_audit_log_action ON kairos_audit.audit_log (action);
CREATE INDEX idx_audit_log_ts     ON kairos_audit.audit_log (ts);

CREATE TABLE kairos_audit.envelope_batches (
...
CREATE TABLE kairos_audit.idempotency_keys (
...
CREATE INDEX idx_idempotency_keys_expires ON kairos_audit.idempotency_keys (expires_at);

-- ----------------------------------------------------------------------------
-- kairos_evolve
-- ----------------------------------------------------------------------------

CREATE TABLE kairos_evolve.routing_policies (
    scope_key       TEXT PRIMARY KEY,
    active_version  BIGINT,
    last_activated_at TIMESTAMPTZ
);

CREATE TABLE kairos_evolve.routing_policy_versions (
    id                  UUID PRIMARY KEY,
...
CREATE INDEX idx_active_policies ON kairos_evolve.routing_policy_versions (scope_key)
    WHERE superseded_at IS NULL AND activated_at IS NOT NULL;

CREATE TABLE kairos_evolve.routing_events (
...
CREATE INDEX idx_routing_events_scope ON kairos_evolve.routing_events (scope_key, received_at);
CREATE INDEX idx_routing_events_batch ON kairos_evolve.routing_events (batch_id);
